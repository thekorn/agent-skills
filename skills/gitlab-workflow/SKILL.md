...
description: Runs repository development through GitLab issues and merge requests—assign @me when starting work, small commits with issue references, push, and MRs that close or reference issues. Use when starting or finishing tracked work, when creating follow-up tasks, or when the user mentions issues, MRs, backlog, or GitLab workflow.
---
...
- Follow the target repository's `AGENTS.md` or equivalent local instructions when present; this skill only defines the GitLab workflow around that repository-specific policy.
